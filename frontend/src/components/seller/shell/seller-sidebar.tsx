  ...
  return (
    <ul className="flex flex-col gap-1">
      {sections.map((section, index) => (
        <li
          key={section.title}
          className={cn(!flush && index > 0 && "mt-5 border-t border-boundary/30 pt-5")}
        >
...
const NavRow = ({
  ...
  <li>
    ...
    >
      {isActive ? (
        <span aria-hidden="true" className="absolute inset-y-1 left-0 w-[3px] rounded-l-control bg-primary motion-safe:animate-fade-in" />
      ) : null}
      ...
      {badgeCount !== null && badgeCount > 0 ? (
        <span
          ...
          className="min-w-5 rounded-full bg-seller-attention-muted px-1.5 py-0.5 text-center text-[11px] font-semibold tabular-nums text-seller-attention"
        >
