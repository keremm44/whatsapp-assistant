...
function SellerWorkbenchProof() {
  return (
    <div className="min-h-[440px] overflow-hidden rounded-sheet border border-boundary/70 bg-chrome shadow-surface">
      ...
      <div className="grid min-h-[360px] lg:grid-cols-[minmax(0,1fr)_320px]">
          ...
          <article className="group relative bg-raised transition-colors duration-200 hover:bg-elevated/50">
            <div className="flex flex-col gap-4 p-5 sm:flex-row sm:items-start sm:gap-5 sm:p-7">
              <div className="flex min-w-0 flex-1 items-start gap-4 sm:gap-5">
                <span aria-hidden="true" className="flex h-10 w-10 shrink-0 items-center justify-center rounded-control border border-boundary/40 bg-recessed text-muted-foreground transition-colors group-hover:bg-hover group-hover:text-foreground">↩</span>
        ...
        <aside className="bg-raised p-5 sm:p-7">
          ...
          <p className="mt-6 type-row-secondary text-muted">
...
function WorkloadLabel({ label, active = false }: { label: string; active?: boolean }) {
  return (
    <div className="relative pb-1">
      <p className={active ? "type-meta font-semibold text-chrome-foreground" : "type-meta text-chrome-foreground/48"}>{label}</p>
      {active ? <span aria-hidden="true" className="absolute inset-x-0 -bottom-0.5 h-px bg-primary" /> : null}
    </div>
  );
}

function ContextRow({ label, value, attention = false }: { label: string; value: string; attention?: boolean }) {
  return (
    <div className="grid grid-cols-[80px_minmax(0,1fr)] gap-3 py-3.5">
      <dt className="type-meta text-muted-foreground">{label}</dt>
      <dd className={attention ? "type-row-secondary font-semibold text-attention" : "type-row-secondary text-foreground"}>{value}</dd>
    </div>
  );
}
