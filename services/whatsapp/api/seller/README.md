...
Domains not yet extracted continue to reuse their original legacy `APIRoute`/handler objects. Extracted domains are registered from their native routers, with focused unit tests patching the new module seams directly. The legacy settings/product/onboarding/conversation definitions remain temporarily in `protected_routes.py` only as compatibility copies; `api.router` does not use those legacy route objects anymore.
