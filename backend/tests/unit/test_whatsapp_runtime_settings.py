...
def test_whatsapp_send_enabled_requires_runtime_first(monkeypatch) -> None:
    monkeypatch.setenv("APP_ENV", "development")
    monkeypatch.delenv("WHATSAPP_RUNTIME_ENABLED", raising=False)
    ...
    get_settings.cache_clear()
    try:
        with pytest.raises(RuntimeError, match="WHATSAPP_RUNTIME_ENABLED"):
            get_settings()
    finally:
        get_settings.cache_clear()
...
def test_whatsapp_send_accepts_explicit_valid_configuration(monkeypatch) -> None:
    ...
    try:
        current = get_settings()
        assert current.whatsapp_runtime_enabled is True
...
def test_invalid_log_level_is_rejected(monkeypatch) -> None:
    monkeypatch.setenv("APP_ENV", "development")
    monkeypatch.setenv("LOG_LEVEL", "INF0")
    monkeypatch.delenv("WHATSAPP_RUNTIME_ENABLED", raising=False)
    _clear_send_env(monkeypatch)
    get_settings.cache_clear()
    try:
...
def test_empty_app_version_is_rejected(monkeypatch) -> None:
    monkeypatch.setenv("APP_ENV", "development")
